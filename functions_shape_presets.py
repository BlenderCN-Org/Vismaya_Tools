...
def category_items(self, context):
    ...
    obj = bpy.context.object
    #try:
    categories = [(cat, cat, '') for cat in obj['shape_preset_data']['_categories_list'].keys() if cat not in ['various', '_old_cat']]       
    #except:
     #   categories = [('All', 'All', '')]      
...
def update_category_list(category):
    ...
    used_categories = [obj['shape_preset_data'][preset]['_category'] for preset in obj['shape_preset_data'].keys() if preset not in ['_categories_list', '_various']]
...
def show_category(self, context):
    ...
    if cat != old_cat:
        ...
        if cat == 'All':
            ...
            presets = [preset for preset in obj['shape_preset_data'].keys() if preset not in ['_categories_list', '_various'] and obj['shape_preset_data'][preset]['_various'] == True]
        else:
            presets = [preset for preset in obj['shape_preset_data'].keys() if preset not in ['_categories_list', '_various'] and obj['shape_preset_data'][preset]['_category'] == cat]
...
def apply_shape_preset(type):
    ...
    if type[0] == 'S': 
        ...
        checked_presets = [preset for preset in obj.shape_preset_list.keys() if obj.shape_preset_list[preset].select_preset]
...
def rename_shape_preset(self, context):
    ...
    """
    obj = bpy.context.object
    if len(obj.shape_preset_list.keys()) > 0 and obj.new_preset_name != obj.shape_preset_list[obj.shape_preset_index].name:
...
def select_all_presets(self, context):
    ...
    presets = [preset for preset in obj.shape_preset_list.keys() if preset != 'All_to_0']
    len_presets = len(presets)
    checked_presets = [preset for preset in obj.shape_preset_list.keys() if preset != 'All_to_0' and obj.shape_preset_list[preset].select_preset == True]
...
def take_snapshot(): 
    ...
    objects_to_hide = [object for object in scn.objects.keys() if not bpy.data.objects[object].hide and object != obj.name]
