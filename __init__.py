...
class OBJECT_PT_PresetNames(bpy.types.PropertyGroup):
    ...
    influence = FloatProperty(min=0.0, max=1.0, default=1.0, precision=3, update=func.preset_influence, description="Influence of preset.")
...
types_enum = [("MODELING","  Modeling","","MOD_ARMATURE",1),("ANIMATION","  Animation","","MOD_SMOOTH",2),("RIGGING","  Rigging","","MOD_SKIN",3),("UVTOOLS","  UV Tools","","MOD_SCREW",4),("LIGHTING","  Lighting","","MOD_EXPLODE",5),("RENDERING","  Rendering","","CAMERA_DATA",6),("DYNAMICS","  Dynamics","","MOD_DYNAMICPAINT",7)]
...
class Vismaya_Shelfs(View3DPanel, bpy.types.Panel):
	...
	def draw(self, context):
		...
		if scn.menu_type == "RIGGING":
			...
			row = layout.row()
			if len(obj.shape_preset_list.keys()) > 0 and obj.shape_preset_list[obj.shape_preset_index].name != 'All_to_0':
...
######## Save Properties ###############
def save_properties():
    bpy.types.Object.preset_category = EnumProperty(items=func.category_items, name='Category', update=func.show_category, description='Select a category of presets.')
    bpy.types.Object.category_name = StringProperty(name='', description='Enter category here to assign it to highlighted preset.', update=func.set_category)                                                       
    ...
    bpy.types.Object.select_all_presets = BoolProperty(name='All', description='Selects all your presets.', update=func.select_all_presets)
    bpy.types.Object.shapekeys_group_mode = BoolProperty(name='Group', description='Group Mode. All shape keys whose <number of first letters> are the same will be saved in preset.')
    bpy.types.Object.group_level_value = IntProperty(name='', min=1, max=10, default=3, description='Number of letters the script has to consider to recognize a group of shape keys.' )
